...
if isempty(ptsFiles)
    error('Khong tim thay file .pts trong thu muc %s', rawDir);
end
...
if ~exist(compressPy, 'file') || ~exist(decompressPy, 'file')
    error('Thieu file Python trong thu muc script');
end

% Luu ket qua vao bang
Results = table();

fprintf('========== BAT DAU XU LY CAC FILE .PTS ==========\n');
...
for f = 1:length(ptsFiles)
    ...
    for s_idx = 1:length(scale_factors)
        ...
        if status ~= 0
            warning('Loi nen cho %s, s=%.4f: %s', name, s, cmdout);
            continue;
        end
        comp_time = toc;
        
        if ~exist(output_ptc, 'file')
            warning('File .ptc khong duoc tao: %s', output_ptc);
            continue;
        end
        ...
        fprintf('  s=%.4f: CR=%.2f, RMSE=%.3f mm, MAE=%.3f mm, nen=%.2fs, giai nen=%.2fs\n', ...
...
disp('========== KET QUA TONG HOP ==========');
disp(Results);

%% Luu bang ket qua ra CSV
csvFile = fullfile(analystDir, 'ket_qua_nen.csv');
writetable(Results, csvFile);
fprintf('Da luu ket qua vao file CSV: %s\n', csvFile);

%% Ve bieu do so sanh CR va RMSE theo scale factor
if height(Results) > 0
    figure('Visible', 'off');
